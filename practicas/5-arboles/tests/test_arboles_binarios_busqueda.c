...
void test_operaciones_varias() {
    ...
    abb_insertar(arbol, te_crear(6));
    abb_insertar(arbol, te_crear(4));
    abb_insertar(arbol, te_crear(9));
    abb_insertar(arbol, te_crear(6));
    mostrar_arbol_binario_ascii(abb_raiz(arbol));
    assert(int_eq(abb_cantidad_elementos(arbol), 7));
...
///    it = iterador_pre_orden(abb_raiz(arbol));
 ...
 //   assert(bool_true(abb_buscar(arbol, 3)));
  ...
  //  assert(bool_true(abb_buscar(arbol, 7)));
    ...
    mostrar_arbol_binario_ascii(abb_raiz(arbol));
    assert(int_eq(abb_cantidad_elementos(arbol), 6));
    assert(bool_false(abb_es_vacio(arbol)));
   ...
   // assert(bool_true(abb_buscar(arbol, 7)));
    ...
    assert(bool_false(abb_es_vacio(arbol)));
    assert(int_eq(abb_raiz(arbol)->datos->clave, 1));
    assert(int_eq(abb_raiz(arbol)->hd->datos->clave, 2));
    assert(int_eq(abb_raiz(arbol)->hd->hd->datos->clave, 3));
    assert(bool_true(abb_buscar(arbol, 2)));
    assert(bool_true(abb_buscar(arbol, 3)));
    assert(bool_true(abb_buscar(arbol, 1)));
    assert(bool_true(abb_buscar(arbol, 7)));
    ...
    abb_eliminar(arbol, 4);
    mostrar_arbol_binario_ascii(abb_raiz(arbol));
    assert(int_eq(abb_cantidad_elementos(arbol), 5));
    assert(bool_false(abb_es_vacio(arbol)));
    assert(int_eq(abb_raiz(arbol)->datos->clave, 1));
    assert(int_eq(abb_raiz(arbol)->hd->datos->clave, 2));
    assert(bool_false(abb_buscar(arbol, 2)));
